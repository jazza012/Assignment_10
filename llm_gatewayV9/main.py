...
DEFAULT_ORDER = ["openai","ollama", "gemini", "nvidia", "groq", "cerebras", "openrouter", "github"]
...
TIER_TO_ORDER = {
    "TINY":  ["openai", "github", "openrouter", "groq", "nvidia", "cerebras", "gemini", "ollama"],
...
def _backoff_for(err: Exception, has_model_override: bool = False):
    ...
    if status == 429:
        import re
        m = re.search(r"try again in (\d+\.?\d*)\s*(s|sec|second|ms)", msg)
        if m:
            val = float(m.group(1))
            unit = m.group(2)
            if unit == "ms":
                val /= 1000.0
            return val + 0.5, f"rate limited (wait {val:.2f}s)"
...
async def chat(req: ChatRequest):
    ...
    if req.auto_route and not req.provider:
        ...
        if req.provider:
            preferred = resolve(req.provider)
            if preferred and preferred in router.providers:
                rest = [p for p in router.order if p != preferred]
    ...
    for _ in range(len(candidates) + 1):
        # Wait up to 45 seconds for a candidate to become available.
        ...
        while time.time() < deadline:
            ...
            for att in atts:
                ...
                if any(x in reason for x in ("RPM", "TPM", "cooldown", "backoff")):
        ...
        router.state[name].in_flight_tokens += est
        try:
            if req.stream:
                async def gen():
                    try:
                        ...
                        router.state[name].in_flight_tokens = max(0, router.state[name].in_flight_tokens - est)
            ...
            try:
                ...
                try:
                    ...
                    sleep_time = 2.0 if status == 429 else min(2.0, 0.5 * (2 ** retries))
                    await _a.sleep(sleep_time)
                    retries += 1
                    result = await provider.chat(messages,
                                                 max_tokens=req.max_tokens,
                                                 temperature=req.temperature,
            ...
            if status == 400 or (status != 401 and status != 403 and status != 429 and status is not None and status < 500 and not getattr(e, "retryable", True)):
