...
class ComputerUseOutput(BaseModel):
    ...
    `layer_used` surfaces which cascade layer(s) handled the task.
    Downstream replay and the Planner's failure routing read this field.
    """

    task: str
    layer_used: str                        # e.g. "L0+L1", "electron_cdp", "L0+L1+L2a+L3"
    content: dict | str | None = None      # extracted or computed result
    note: str = ""                         # human-readable cascade note
    actions: list[dict] = Field(default_factory=list)
    traj_dir: str = ""                     # absolute path to trajectory directory
