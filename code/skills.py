...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "computer_use":
        # Session 10: Computer-Use skill — 5-layer cascade over the OS.
        # Owns its own drivers (shell / hotkeys / UIA / AX+LLM / vision)
        # and records every run to state/trajectories/. Bypasses the
        # gateway-chat dispatch exactly like the Browser skill does.
        node_dict = graph_nodes[node_id]
        node_spec = NodeSpec(
            skill="computer_use",
            inputs=node_dict.get("inputs") or [],
